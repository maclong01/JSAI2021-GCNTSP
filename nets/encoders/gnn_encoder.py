...
class GNNLayer(nn.Module):
    ...
    def __init__(self, hidden_dim, aggregation="sum", norm="batch", learn_norm=True, track_norm=False, gated=True):
        ...
        self.rnn_h = torch.nn.LSTM(input_size = hidden_dim, 
                                 hidden_size = hidden_dim,
                                 num_layers = 3, 
                                 batch_first = True,)

        self.rnn_e = torch.nn.LSTM(input_size = hidden_dim, 
                                 hidden_size = hidden_dim,
                                 num_layers = 3, 
                                 batch_first = True,)
        ...
        self.dense_h= torch.nn.Sequential(
            torch.nn.Linear(hidden_dim, hidden_dim),
            torch.nn.ReLU(),
        )

        self.dense_e= torch.nn.Sequential(
            torch.nn.Linear(hidden_dim, hidden_dim),
            torch.nn.ReLU(),
        )
    ...
    def forward(self, h, e, graph):
        ...
        #LSTM_h1
        h_lstm1, _ = self.rnn_h(h)
        h_lstm1 = self.dense_h(h_lstm1)
        #LSTM_h2
        h_lstm2, _ = self.rnn_h(h)
        h_lstm2 = self.dense_h(h_lstm2)

        Ce = self.C(e)  # B x V x V x H

        #Dense
        Ce = self.dense_e(
            Ce.view(batch_size*num_nodes*num_nodes, hidden_dim)
        ).view(batch_size, num_nodes, num_nodes, hidden_dim)



        # Update edge features and compute edge gates
        e = Ah.unsqueeze(1) + Bh.unsqueeze(2) + Ce +h_lstm1.unsqueeze(1) ++h_lstm2.unsqueeze(2)# B x V x V x H
